...
# Excel file path (This file will be created/updated in your project folder)
...
with col_confirm:
    ...
    if st.button("💾 Save Booking", use_container_width=True):
        ...
        try:
            ...
            if delta.total_seconds() < 0:
                 # Add one day if end time is numerically before start time (e.g., booking from 11 PM to 1 AM)
            ...
            # Final Calculation and Save
                ...
                if os.path.exists(EXCEL_FILE):
                    try:
                        ...
                        st.error("❌ ERROR: Required module 'openpyxl' not found. Please run: pip install openpyxl")
                        st.stop()
                    except Exception as e:
                        # Fallback if file is unreadable
                        st.warning(f"Existing Excel file could not be read. Starting a new log. Error: {e}")
    ...
    try:
        start_dt_calc = datetime.strptime(start_time.strip().upper(), "%I:%M %p")
        end_dt_calc = datetime.strptime(end_time.strip().upper(), "%I:%M %p")
        ...
        price_calc = round(hours_calc * PRICES[table], 2)
        
        current_price_placeholder.info(f"Hourly Rate: **₹{PRICES[table]}** | Est. Price: **₹{price_calc:,.2f}** for {round(hours_calc, 2)} hours")
...
with col_bookings:
    ...
    if os.path.exists(EXCEL_FILE):
        try:
            ...
            st.error(f"Error displaying bookings: {e}")
    else:
...
<div style='font-size: small; color: #555;'>
    ℹ️ **File Location:** The booking data is stored in the Excel file 
    `{EXCEL_FILE}` located in the same directory as this `app.py` file:
    <br/>
